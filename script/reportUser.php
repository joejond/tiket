<?php
	...
	while($data = mysql_fetch_array($rs))
	{
		$worksheet1->set_column(1,$i,25);
		$worksheet1->write_string(1,$i,$data['Ticketlist'],$format);
		$i++;
	}
	if($IMEI==1)
	{
		$worksheet1->set_column(1,$i,25);
		$worksheet1->write_string(1,$i,"Serial Number",$format);
		$i++;
	}
	if($RIM==1)
	{
		$worksheet1->set_column(1,$i,25);
		$worksheet1->write_string(1,$i,"External Ticket",$format);
		$i++;
	}
	...
	$worksheet1->set_column(1,$i,25);
	$worksheet1->write_string(1,$i,"Open Date",$format);
	$i++;
	$worksheet1->set_column(1,$i,25);
	$worksheet1->write_string(1,$i,"Escalate Date",$format);
	$i++;
	$worksheet1->set_column(1,$i,25);
	$worksheet1->write_string(1,$i,"Confirmed Date",$format);
	$i++;
	$worksheet1->set_column(1,$i,25);
	$worksheet1->write_string(1,$i,"Response Time",$format);
	$i++;
	$worksheet1->set_column(1,$i,25);
	$worksheet1->write_string(1,$i,"Resolution Time",$format);
	$i++;
	$worksheet1->set_column(1,$i,25);
	$worksheet1->write_string(1,$i,"Summary Code",$format);
	$i++;
	...
	$baris = 2;
	$y = 0;
	$query = "select * from MsPengaduan where Bec = '".$bec."' and Pengaduan = '".$_POST['ticket']."'";
	$rs = mysql_query($query);
	while($data = mysql_fetch_array($rs))
	{
		
		if($_POST['status']=="ALL") $query2 = "select *,hour(timediff(Escalatedate,Opendate)) as Aging1,hour(timediff(Closedate,Escalatedate)) as Aging2 from `".$data['Nomorticket'].":".$data['Bec']."` 
					where Opendate between '".$date_from."' and '".$date_to."'";
		else if($_POST['status']=="OPEN") $query2 = "select *,hour(timediff(Escalatedate,Opendate)) as Aging1,hour(timediff(Closedate,Escalatedate)) as Aging2 from `".$data['Nomorticket'].":".$data['Bec']."` 
					where Opendate between '".$date_from."' and '".$date_to."' and Escalatedate = '00-00-0000 00:00:00' and Closedate = '00-00-0000 00:00:00'";
		else if($_POST['status']=="ESCALATE") $query2 = "select *,hour(timediff(Escalatedate,Opendate)) as Aging1,hour(timediff(Closedate,Escalatedate)) as Aging2 from `".$data['Nomorticket'].":".$data['Bec']."` 
					where Opendate between '".$date_from."' and '".$date_to."' and Escalatedate != '00-00-0000 00:00:00' and Closedate = '00-00-0000 00:00:00'";
		else if($_POST['status']=="CLOSE") $query2 = "select *,hour(timediff(Escalatedate,Opendate)) as Aging1,hour(timediff(Closedate,Escalatedate)) as Aging2 from `".$data['Nomorticket'].":".$data['Bec']."` 
					where Opendate between '".$date_from."' and '".$date_to."' and Escalatedate != '00-00-0000 00:00:00' and Closedate != '00-00-0000 00:00:00'";
