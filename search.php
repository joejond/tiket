		...
		?>
            ...
            method="post">
                <table>
                    <tr>
                        <td>
                            Insert External Ticket :
                        </td>
                        <td>
                        	<input type="text" name="RIM" style="width:150px" />
                        </td>
                        <td>
                            <input type="submit" value="Escalate" style="width:100px" />
                        </td>
                    </tr>
                    <tr>
        ...
        <form action="script/close.php?ID=<?php echo $_REQUEST['ID']; ?>&BEC=<?php echo $_REQUEST['BEC']; ?>" method="post">
                <table>
                    <tr>
                        ...
                        <td>
                        	<select name="SummaryCode">
                            	<option value="-">Choose Summary Code</option>
                            	<?php
									$query = "select Code,Description from `".$pengaduan.":summary` order by Code";
									$rs = mysql_query($query);
									while($data = mysql_fetch_array($rs))
									{
								?>
                                		<option value="<?php echo $data['Code']." : ".$data['Description']; ?>">
										<?php echo $data['Code']." : ".$data['Description']; ?></option>
                                <?php
									}
								?>
                            </select>
                        ...
                        <td>
                        	<input type="submit" value="Close" style="width:100px" />
                        </td>
                    </tr>
                    <tr>
		...
		<form action="script/sendTicket.php?BEC=<?php echo $_REQUEST['BEC']; ?>" method="post">
        	...
        	<table>
                ...
                <?php
							...
							else if($sent==0 && $status!="USER" && $escalate=="0000-00-00 00:00:00" && $close=="0000-00-00 00:00:00")
							{
				?>
                			<tr>
                                <td width="300" align="left"><?php echo "Serial Number"; ?></td>
                                <td width="400" align="left"><input type="text" style="width:390px" rows="1"
                                value="<?php echo $data['Imei']; ?>" name="IMEI" />
                                </td>
                            </tr>
                            <tr height="5"></tr>
                <?php			
				...
				?>
                            <tr>
                                <td width="300" align="left"><?php echo $data['Text list '.$i]; ?></td>
                                <td width="400" align="left"><textarea style="width:390px" value="<?php echo $data['Text value '.$i]; ?>" name="TL<?php echo $i; ?>"><?php echo $data['Text value '.$i]; ?></textarea>
                                </td>
                            </tr>
                            <tr height="5"></tr>
                <?php	
                ...
                <tr>
                  ...
                  <tr>
                      ...
                      <p id="buttonClick"> </p>
                          <input type="submit" name="Editing" value="Edit" onclick='buttonClicked(this)' style="width:120px" />
                      <?php
					  	if($email!="" && $email!=" ")
						{
					  ?>
                      		<input type="submit" name="Editing" value="Send" onclick='buttonClicked(this)' style="width:120px" />
                      <?php
						}
					  ?></td>
                  </tr>
...
?>
        <form action="script/comment.php?ID=<?php echo $_REQUEST['ID']; ?>&BEC=<?php echo $_REQUEST['BEC']; ?>" method="post" enctype="multipart/form-data">
            <table style="background-color:#CCCCCC;color:#666666">
    <?php
            $query = "select * from MsPengaduan where Nomorticket = '".$_REQUEST['ID']."'";
            $rs = mysql_query($query);
            while($data = mysql_fetch_array($rs))
    ...
    ?>
                <tr>
                    <td colspan="2" style="border-bottom:thin solid #999999">
    ...
    ?>
                <tr>
                    <td width="400"><textarea name="comment" style="width:400px"></textarea></td>
                    <td><input type="submit" value="Send" style="width:80px"/></td>
                </tr>
                <tr>
                	<td colspan="2"><input type="file" name="file" style="width:400px;height:20px" /></td>
                </tr>
            </table>
		</form>
<?php
